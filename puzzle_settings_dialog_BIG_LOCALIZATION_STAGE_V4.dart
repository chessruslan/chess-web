...
class _PuzzleSettingsDialogState extends State<PuzzleSettingsDialog> {
  ...
  Future<void> _publish() async {
    ...
    if (puzzle.startFen.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: MakeChessLocalizedText('Сначала запишите начальную позицию')),
      );
      return;
    }
    if (puzzle.solutionLines.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: MakeChessLocalizedText('Сначала запишите хотя бы одну ветку')),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: MakeChessLocalizedText('Задача опубликована в выбранную папку')),
      );
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _saveToComputer() async {
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: MakeChessLocalizedText('Файл задачи скачан на компьютер')),
    );
  }
