...
String _openingWordRu(String source) {
  ...
  final normalized =
      original.replaceAll('’', "'").replaceAll('`', "'").toLowerCase();
...
class OpeningBotReplyRuleParser {
  ...
  static bool matches(String source, int botReplyNumber) {
    ...
    for (final range in parseRanges(source)) {
      final start = range.$1;
      final end = range.$2;
      if (botReplyNumber >= start && (end == null || botReplyNumber <= end)) {
        return true;
      }
    }
    return false;
...
class OpeningTree {
  ...
  factory OpeningTree.fromJson(Map<String, dynamic> json) {
    ...
    final name =
        '${json['name'] ?? json['title'] ?? 'Загруженный дебют'}'.trim();
    final studentColor =
        '${json['studentColor'] ?? json['student_color'] ?? 'white'}'
            .trim()
            .toLowerCase();
    ...
    return OpeningTree(
      ...
      sourceName:
          '${json['sourceName'] ?? json['source_name'] ?? ''}'.trim().isEmpty
              ? null
              : '${json['sourceName'] ?? json['source_name']}',
      sourceLicense: '${json['sourceLicense'] ?? json['source_license'] ?? ''}'
              .trim()
              .isEmpty
          ? null
...
// MAKECHESS_OPENING_DISPLAY_NAME_V5_20260807
String _openingDisplayName(OpeningDatabaseItem item) {
  final databaseName = item.databaseName.trim();
  if (MakeChessLocalizationController.currentCode == 'RU' ||
...
String _openingFamilyName(String name) {
  ...
  return (colon < 0 ? name : name.substring(0, colon)).trim().toLowerCase();
}
              ...
              for (var row = 0; row < 8; row++)
                for (var column = 0; column < 8; column++)
                  Positioned(
                    ...
                    child: Builder(
                      builder: (context) {
                        final actualFile = widget.flipped ? 7 - column : column;
          ...
          final details = Column(
            ...
            children: <Widget>[
              ...
              Wrap(
                ...
                children: <Widget>[
                  AppNeoButton(
                    text: MakeChessLocalization.phrase(
                        _playing ? 'Пауза' : 'Продолжить'),
...
class OpeningTrainerController extends ChangeNotifier {
  ...
  List<OpeningDatabaseItem> _matchingOpenings = const <OpeningDatabaseItem>[];
  ...
  List<OpeningEngineLine> get rawEngineLines =>
      List.unmodifiable(_rawEngineLines);
  ...
  Future<OpeningDatabaseItem> fetchPublishedOpeningFamily(
    ...
    try {
      ...
      final familyRoot =
          selectedDatabaseName.split(RegExp(r'[:,]')).first.trim();
      ...
      while (true) {
        ...
        for (final row in rows) {
          ...
          final selectedIsFamilyRoot = selectedDatabaseName == familyRoot;
  ...
  Future<void> analyzePosition({
    ...
    try {
      ...
      if (treeRequired && node != null && node.moves.isNotEmpty) {
        ...
        final globalMoveSet = _rawEngineLines.map((line) => line.uci).toSet();
  ...
  OpeningMoveSuggestion _chooseProgrammedBotSuggestion(
    ...
    final selected =
        targetIndex < sorted.length ? sorted[targetIndex] : sorted.first;
      ...
      try {
        ...
        for (final entry in current.moves.entries) {
          if (hasRealPopularity) {
            weights[entry.key] = (weights[entry.key] ?? 0) + item.popularity;
          } else {
  ...
  void _refreshMatchingOpenings({bool announce = true}) {
    ...
    final signature =
        '${_historyUci.join(' ')}|${matching.map((e) => e.id).join(',')}';
  ...
  List<OpeningMoveSuggestion> _filterEngineSuggestions(
    ...
    final filtered = lines
        .where((line) =>
            bestScore - line.score <= _settings.maxEvaluationDrop + 1e-9)
  ...
  List<OpeningMoveSuggestion> _buildOpeningSuggestions(
    ...
    final accepted = scored
        .where(
          (item) =>
              bestScore - item.score <= _settings.maxEvaluationDrop + 1e-9,
        )
        .take(_settings.maxVariants)
...
class _OpeningTrainerDialogState extends State<OpeningTrainerDialog> {
    ...
    return Container(
      ...
      child: Row(
        children: <Widget>[
          ...
          _headerButton(
            tooltip:
                MakeChessLocalization.phrase('Настройки дебютного тренажёра'),
              ...
              for (final item in items.take(4))
                MakeChessLocalizedText(
                  ...
                  style: AppTextStyles.caption.copyWith(
                    ...
                    fontWeight:
                        item.rank == 1 ? FontWeight.w700 : FontWeight.w500,
                  ),
                ),
            ],
          ),
  ...
  Future<void> _openOpeningChooser(
    ...
    List<_OpeningPreviewBranch> previewBranches = <_OpeningPreviewBranch>[];
    ...
    openPreview = (OpeningDatabaseItem selected) async {
      ...
      try {
        ...
        updateDialog(() {
          ...
          previewBranches = List<_OpeningPreviewBranch>.unmodifiable(branches);
          previewVisibleCount = branches.length < 4 ? branches.length : 4;
          page = 'preview';
        });
      } catch (error) {
        if (!dialogIsOpen || currentRequest != requestNumber) return;
      ...
      return SingleChildScrollView(
        child: Column(
          ...
          children: <Widget>[
            ...
            MakeChessLocalizedText('За чёрных',
            ...
            const SizedBox(height: 9),
            MakeChessLocalizedText('Автоматические режимы',
                style: AppTextStyles.panelTitle),
            const SizedBox(height: 7),
            modeButton(
      ...
      final visible = normalizedQuery.isEmpty
          ...
          : shownOpenings.where((item) {
              return item.name.toLowerCase().contains(normalizedQuery) ||
                  item.firstMovesLabel.toLowerCase().contains(normalizedQuery);
            }).toList(growable: false);
        ...
        children: <Widget>[
          TextField(
            ...
            decoration: AppInputs.dark(
              ...
              hintText:
                  MakeChessLocalization.phrase('Например: Сицилианская защита'),
            ),
          ),
          const SizedBox(height: 10),
          MakeChessLocalizedText(
                        ...
                        trailing: Wrap(
                          ...
                          children: <Widget>[
                            TextButton.icon(
                              ...
                              label:
                                  const MakeChessLocalizedText('Предпросмотр'),
                            ),
                            IconButton(
    ...
    Widget previewPage() {
      ...
      final visibleBranches =
          previewBranches.take(safeVisibleCount).toList(growable: false);
                    ...
                    AppNeoButton(
                      ...
                      onTap: () {
                        updateDialog(() {
                          ...
                          previewVisibleCount =
                              next > totalBranchCount ? totalBranchCount : next;
                        });
                      },
                      compact: true,
                    ),
                    ...
                    children: <Widget>[
                      ...
                      TextField(
                        ...
                        decoration: AppInputs.dark(
                          ...
                          helperText: MakeChessLocalization.phrase(
  ...
  Future<void> _openBotSettings(
    ...
    String? validate({
      ...
      final treeError = OpeningBotReplyRuleParser.validateRuleSet(treeRules);
      if (treeError != null) {
              ...
              return Container(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                    ...
                    MakeChessLocalizedText(description,
                        style: AppTextStyles.caption),
                    const SizedBox(height: 10),
                    for (var line = 1; line <= 5; line++) ...<Widget>[
                      TextField(
                        ...
                        decoration: AppInputs.dark(
                          ...
                          hintText:
                              line == 1 ? 'все' : 'например 3,5,7 или 4-8',
                          helperText: line == 1
              ...
              actions: <Widget>[
                ...
                SizedBox(
                  ...
                  child: AppNeoButton(
                    ...
                    icon: Icons.rule,
                    onTap: () {
                      final cycles = int.tryParse(cyclesCtl.text.trim()) ?? 0;
                      final games = int.tryParse(gamesCtl.text.trim()) ?? 0;
                      final identical =
                ...
                SizedBox(
                  ...
                  child: AppNeoButton(
                    ...
                    icon: Icons.save_outlined,
                    onTap: () {
                      final cycles = int.tryParse(cyclesCtl.text.trim());
                      final games = int.tryParse(gamesCtl.text.trim());
                      final identical = int.tryParse(identicalCtl.text.trim());
                      final depth = int.tryParse(depthCtl.text.trim());
                      ...
                      if (cycles == null ||
                          ...
                          delay == null) {
                        setLocalState(() {
                          localError = 'Проверьте числовые значения настроек.';
                          localSuccess = null;
                        });
                        return;
                      }
