...
class _TournamentGamePlatformDialogState
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: MakeChessLocalizedText(
                    'Не удалось завершить партию: $error')),
          );
        }
        return;
      }
  ...
  Future<void> _openLiveTournamentTable() async {
    if (widget.tournamentId.isEmpty || widget.ownerId.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: MakeChessLocalizedText('Сначала выберите турнир')),
      );
      return;
    }
    final storage = TournamentStorageService.instance;
  ...
  Widget _layoutToolbar() {
    return Container(
      ...
      child: Row(
        children: [
          const MakeChessLocalizedText('Компоновка:',
  ...
  Widget _controls() {
    ...
    final analysisAction = _gameFinished
        ? () => ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                  content: MakeChessLocalizedText('Инструмент анализа открыт')),
            )
        : null;
    return Container(
      padding: const EdgeInsets.all(10),
...
class _LiveTournamentTableDialogState
  ...
  Widget build(BuildContext context) {
    ...
    final pairings = tournament?['pairings'] is List
        ...
        : <Map<String, dynamic>>[];
    ids.sort(
        (a, b) => _pointsFor(b, pairings).compareTo(_pointsFor(a, pairings)));
          ...
          children: [
            ListTile(
              ...
              title: MakeChessLocalizedText(
                  'Турнирная таблица — ${widget.tournamentName}'),
            ...
            if (_loading)
              ...
              const Expanded(
                child: Center(
                    child:
                        MakeChessLocalizedText('Не удалось загрузить турнир')),
              )
            else
              Expanded(
                child: SingleChildScrollView(
