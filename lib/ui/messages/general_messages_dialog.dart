...
final ValueNotifier<int> makechessAdminReplyUnreadCount = ValueNotifier<int>(0);
...
class _GeneralMessageRecipient {
  const _GeneralMessageRecipient({
    required this.id,
    required this.name,
    required this.kind,
    this.subtitle = '',
...
class _GeneralMessagesDialogState extends State<_GeneralMessagesDialog> {
  ...
  List<_GeneralMessageRecipient> _userRecipients = <_GeneralMessageRecipient>[];
  List<_GeneralMessageRecipient> _schoolRecipients =
      <_GeneralMessageRecipient>[];
  _GeneralMessageRecipient? _selectedRecipient;
  MakeChessMessage? _replySource;
  ...
  Future<void> _reload() async {
    ...
    // Surgical rule: sent tournament messages are NOT added here.
    // We fetch only ordinary correspondence sent by the current user.
    final sentDirect = <MakeChessMessage>[];
    try {
      final rows = await Supabase.instance.client
          ...
          .inFilter(
        'category',
        <String>['direct_message', 'admin_contact', 'admin_case_reply'],
      ).order('created_at', ascending: false);
      sentDirect.addAll(
        (rows as List).whereType<Map>().map(
              (row) => MakeChessMessage.fromDatabaseJson(
                Map<String, dynamic>.from(row),
              ),
            ),
      );
    ...
    final merged = <String, MakeChessMessage>{
      ...
      for (final message in sentDirect) message.id: message,
    }.values.toList()
      ..sort((a, b) => b.createdAt.compareTo(a.createdAt));
          ...
          .where(
            (message) =>
                ...
                (message.senderId == widget.currentUserId &&
                    (message.category == 'direct_message' ||
                        message.category == 'admin_contact' ||
                        message.category == 'admin_case_reply')),
          )
          .toList(growable: false);
      _loading = false;
  ...
  bool _isIncomingMessage(MakeChessMessage message) =>
      message.recipientId == widget.currentUserId || message.recipientId == '*';
  ...
  Future<void> _loadRecipientDirectory() async {
    if (mounted) setState(() => _directoryLoading = true);
    try {
      ...
      final users = <_GeneralMessageRecipient>[];
      final rawProfiles =
          results[0] is List ? results[0] as List : const <dynamic>[];

      for (final raw in rawProfiles.whereType<Map>()) {
        final row = Map<String, dynamic>.from(raw);
        final id = '${row['id'] ?? ''}'.trim();
        if (id.isEmpty || id == widget.currentUserId) continue;
          ...
          _GeneralMessageRecipient(
            ...
            subtitle: <String>[
              if (rating.isNotEmpty)
                MakeChessLocalization.phrase(
                  'Рейтинг: {rating}',
                  params: <String, Object?>{'rating': rating},
                ),
      ...
      final schools = <_GeneralMessageRecipient>[];
      ...
      for (final dynamic account in accounts) {
        ...
        final recipientId = '${account.ownerUserId}'.trim();
        if (schoolName.isEmpty || recipientId.isEmpty) continue;

        // MAKECHESS_SCHOOLS_VISIBLE_V8_6_3_20260808
        // A registered school must stay visible even when its owner is the
        // currently logged-in user (for example while testing in teacher mode).
        final key = '$recipientId::$schoolName';
        if (!seenSchools.add(key)) continue;
        ...
        schools.add(
          _GeneralMessageRecipient(
            ...
            name: schoolName,
            kind: 'school',
            subtitle: MakeChessLocalization.phrase('Школа'),
          ),
        );
      }
      ...
      if (!mounted) return;
      setState(() {
        _userRecipients = users;
        _schoolRecipients = schools;
  ...
  _GeneralMessageRecipient? get _administrationRecipient {
    final candidates = _messages
        .where(
          (message) =>
              message.senderId != widget.currentUserId &&
              message.senderId.trim().isNotEmpty &&
              (message.category == 'admin_direct' ||
    ...
    return _GeneralMessageRecipient(
      id: candidates.first.senderId,
      name: MakeChessLocalization.phrase('Администратор'),
      kind: 'administration',
      subtitle: MakeChessLocalization.phrase(
        'Сообщение будет направлено администрации',
      ),
    );
  }

  List<_GeneralMessageRecipient> get _visibleRecipients {
    final source = switch (_recipientMode) {
      'schools' => _schoolRecipients,
      'administration' => <_GeneralMessageRecipient>[
          if (_administrationRecipient != null) _administrationRecipient!,
        ],
      _ => _userRecipients,
    };

    final query = _recipientSearch.text.trim().toLowerCase();
    if (query.isEmpty) return source;
    return source
        .where(
          (recipient) =>
              recipient.name.toLowerCase().contains(query) ||
              recipient.subtitle.toLowerCase().contains(query),
        )
        .toList(growable: false);
  }

  _GeneralMessageRecipient? _replyRecipientFor(MakeChessMessage message) {
    final isMine = message.senderId == widget.currentUserId;

    if (isMine) {
      final recipientId = message.recipientId.trim();
      if (recipientId.isEmpty || recipientId == '*') return null;
      final recipientName =
          '${message.metadata['recipientName'] ?? message.metadata['targetName'] ?? recipientId}'
              .trim();
      return _GeneralMessageRecipient(
        id: recipientId,
        name: recipientName.isEmpty ? recipientId : recipientName,
        kind: '${message.metadata['recipientKind'] ?? 'user'}',
      );
    }
    ...
    final isAdministration = message.category == 'admin_direct' ||
        message.category == 'admin_info' ||
        message.category == 'admin_warning' ||
        message.category == 'admin_block' ||
        message.category == 'admin_restriction' ||
    ...
    // IMPORTANT: for a tournament invitation senderId is the tournament
    // creator/organizer. We only use it as the reply address.
    // No tournament status/action code is changed here.
    return _GeneralMessageRecipient(
      id: senderId,
      name: isAdministration
          ? MakeChessLocalization.phrase('Администратор')
          : message.senderName,
      kind: isAdministration ? 'administration' : 'user',
      subtitle: message.category == 'tournament_invite'
          ? MakeChessLocalization.phrase('Организатор турнира')
          : '',
    );
  }

  void _prepareReplyToMessage(MakeChessMessage message) {
    final recipient = _replyRecipientFor(message);
    if (recipient == null || !recipient.isValid) return;
    setState(() {
      _selectedRecipient = recipient;
      _replySource = message;
    });
  }

  void _selectRecipient(_GeneralMessageRecipient recipient) {
    setState(() {
      _selectedRecipient = recipient;
      _replySource = null;
    });
  ...
  String _messagePartyLine(MakeChessMessage message) {
    if (_isIncomingMessage(message)) {
      return MakeChessLocalization.phrase(
        'От: {name}',
    ...
    final recipient =
        '${message.metadata['recipientName'] ?? message.recipientId}'.trim();
    return MakeChessLocalization.phrase(
      'Кому: {name}',
      params: <String, Object?>{
        'name': recipient.isEmpty ? message.recipientId : recipient,
      },
    );
  }
  ...
  Future<void> _sendComposedMessage() async {
    ...
    if (recipient == null || !recipient.isValid || body.isEmpty) return;
    if (_sendingDirect) return;
    ...
    try {
      final source = _replySource;

      // Administrative case reply keeps the EXISTING case mechanism.
      if (source != null && _isAdminCaseMessage(source)) {
        ...
        final toAdministration = recipient.kind == 'administration';
        await MakeChessMessageRealtimeService.instance.start(client);
        await MakeChessMessageRealtimeService.instance.send(
          MakeChessMessage(
            id: 'direct_${DateTime.now().microsecondsSinceEpoch}',
            recipientId: recipient.id,
            senderId: user.id,
            senderName: widget.currentUserName,
            category: toAdministration ? 'admin_contact' : 'direct_message',
            title: toAdministration
                ? 'Сообщение администрации'
                : 'Личное сообщение',
            body: body,
            createdAt: DateTime.now(),

            // Deliberately NOT setting tournamentId here.
            // A written reply to a tournament invitation must never trigger
            // or replace tournament participation mechanics.
            metadata: <String, dynamic>{
              'recipientName': recipient.name,
              'recipientKind': recipient.kind,
              'sourceMessageId': source?.id ?? '',
              'sourceCategory': source?.category ?? '',
              'sourceTournamentId': source?.tournamentId ?? '',
              if (toAdministration) ...<String, dynamic>{
      ...
      setState(() => _replySource = null);
      await _reload();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            MakeChessLocalization.phrase('Сообщение отправлено'),
          ),
        ),
      );
    } catch (error) {
      ...
      if (mounted) setState(() => _sendingDirect = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final dialogHeight =
        (MediaQuery.sizeOf(context).height * .91).clamp(690.0, 820.0);
    ...
    return Dialog(
      ...
      child: Container(
        ...
        child: Column(
          children: [
            // ZONE 1: title + existing filters.
            _header(),
            _filters(),
            const Divider(height: 1, color: Colors.white12),

            // ZONE 2: existing messages. Tournament tiles/actions stay intact.
            Expanded(
              flex: 5,
              child: _messageListZone(visible),
            ),
            const Divider(height: 1, color: Colors.white12),

            // ZONE 3: recipient search.
            SizedBox(
              height: 235,
              child: _recipientSearchZone(),
            ),
            const Divider(height: 1, color: Colors.white12),

            // ZONE 4: compose new/reply message.
            SizedBox(
              height: 175,
              child: _composeZone(),
            ),
  ...
  Widget _messageListZone(List<MakeChessMessage> visible) {
    if (_loading) {
    ...
    return ListView.separated(
      ...
      itemBuilder: (_, index) {
        ...
        final incomingUnread =
            _isIncomingMessage(message) && message.status == 'unread';

        return ListTile(
          dense: true,
          tileColor: incomingUnread
              ? Colors.cyanAccent.withOpacity(.08)
              : Colors.white.withOpacity(.025),
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(10),
            side: BorderSide(
              color: incomingUnread
          ...
          leading: Icon(
            message.category.startsWith('tournament')
                ...
                : message.senderId == widget.currentUserId
                    ? Icons.send_outlined
                    : Icons.mail_outline,
            color: incomingUnread ? Colors.cyanAccent : Colors.white54,
          ),
          title: MakeChessLocalizedText(
            _makeChessLocalizedMessageTitle(message),
          ),
          subtitle: MakeChessLocalizedText(
            '${_messagePartyLine(message)} • ${_date(message.createdAt)}\n'
            '${_makeChessLocalizedMessageBody(message)}',
            maxLines: 2,
            overflow: TextOverflow.ellipsis,
          ),

          // EXISTING tournament trailing controls copied without semantic change.
          trailing: message.category == 'tournament_join_request' &&
                  message.status != 'accepted' &&
                  message.status != 'declined'
              ? ActionChip(
                  avatar: const Icon(Icons.person_add, size: 16),
                  label: MakeChessLocalizedText(
                    MakeChessLocalization.phrase('Принять участника'),
                  ),
                  onPressed: () async {
              ...
              : _isTournamentCall(message)
                  ? ActionChip(
                      ...
                      onPressed: () => _openTournamentPlatform(message),
                    )
                  : message.status == 'accepted'
                      ? message.category == 'tournament_invite' &&
                              message.tournamentId != null
                          ? ActionChip(
                              avatar: const Icon(
                              ...
                              onPressed: () => _previewTournament(message),
                            )
                          : Chip(
                              label: MakeChessLocalizedText(
                                MakeChessLocalization.phrase('Принято'),
                              ),
                            )
                      : incomingUnread
                          ? const Icon(
                          ...
                          : null,
          onTap: () async {
            // This only preselects the correspondent for a quick written reply.
            // Existing tournament dialogs/actions continue unchanged.
            _prepareReplyToMessage(message);
  ...
  Widget _recipientSearchZone() {
    ...
    Widget modeChip(String key, String label, IconData icon) {
      return ChoiceChip(
        selected: _recipientMode == key,
        avatar: Icon(icon, size: 16),
        label: Text(MakeChessLocalization.phrase(label)),
        onSelected: (_) {
          setState(() {
            _recipientMode = key;
            ...
            // Existing administrator-selection behavior is preserved.
    ...
    return Padding(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          // MAKECHESS_SEARCH_RESULTS_LIST_V8_6_2_20260808
          // Search field stays on the SAME row as recipient categories.
          Row(
            children: [
              Text(
                MakeChessLocalization.phrase('Кому написать'),
                style: const TextStyle(
                  color: Colors.white,
              ...
              modeChip('users', 'Пользователи', Icons.person_outline),
              const SizedBox(width: 6),
              modeChip('schools', 'Школы', Icons.school_outlined),
              const SizedBox(width: 6),
              modeChip(
                'administration',
                'Администрация',
                ...
                child: SizedBox(
                  ...
                  child: TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      hintText:
                          MakeChessLocalization.phrase('Поиск получателя'),
                      border: const OutlineInputBorder(),
                      isDense: true,
                    ),
                  ),
              ...
              const SizedBox(width: 6),
              IconButton(
                tooltip: MakeChessLocalization.phrase('Обновить списки'),
                onPressed: _directoryLoading ? null : _loadRecipientDirectory,
          ...
          // Search results are shown VERTICALLY in one larger scrollable field.
          Expanded(
            child: Container(
              decoration: BoxDecoration(
                              ...
                              child: Scrollbar(
                                child: ListView.separated(
                                  ...
                                  itemBuilder: (_, index) {
                                    ...
                                    final selected = _selectedRecipient?.id ==
                                            recipient.id &&
                                        _selectedRecipient?.kind ==
                                            recipient.kind;
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Text(
                MakeChessLocalization.phrase('Новое сообщение'),
                style: const TextStyle(
                  color: Colors.white,
                  fontWeight: FontWeight.w800,
                ),
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: recipient == null
                        ...
                        : Colors.cyanAccent.withOpacity(.07),
                    borderRadius: BorderRadius.circular(8),
                    border: Border.all(
                      color: recipient == null
                          ? Colors.white12
          ...
          Expanded(
            child: TextField(
              ...
              onChanged: (_) => setState(() {}),
              decoration: InputDecoration(
                hintText: _replySource == null
                    ? MakeChessLocalization.phrase('Напишите сообщение')
                    : MakeChessLocalization.phrase('Напишите ответ'),
                border: const OutlineInputBorder(),
              ),
            ),
          ...
          Align(
            ...
            child: FilledButton.icon(
              onPressed: canSend ? _sendComposedMessage : null,
              icon: _sendingDirect
                  ? const SizedBox(
                      width: 16,
                      height: 16,
              ...
              label: Text(MakeChessLocalization.phrase('Отправить')),
            ),
          ),
        ],
      ),
