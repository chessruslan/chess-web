...
class AdminModerationStore {
  ...
  static const String _deletedTargetsKey =
...
class _AdminManagementPanelState extends State<AdminManagementPanel> {
  ...
  Future<void> _load() async {
    ...
    setState(() {
      ...
      if (_selected == null && targets.isNotEmpty) {
        ...
        _selected = matches.isEmpty ? (targets.isEmpty ? null : targets.first) : matches.first;
      }
      _loading = false;
    });
  ...
  Future<List<AdminTarget>> _loadPlayers() async {
    ...
    for (final item in rows.whereType<Map>()) {
      ...
      final name = '${row['nickname'] ?? row['name'] ?? row['email'] ?? id}'.trim();
    ...
    for (final raw in rows) {
      ...
      result.add(
        AdminTarget(
          ...
          subtitle:
              '${status.isEmpty ? _t('Статус не указан') : _t(status)} • ${_t('{count} участников', params: <String, Object?>{'count': count})}',
          raw: row,
        ),
      );
    }
  ...
  List<AdminCaseRecord> get _selectedCases {
    ...
    final result =
        _cases.where((item) => item.targetId == target.id).toList(growable: false);
    result.sort((a, b) => b.updatedAt.compareTo(a.updatedAt));
    return result;
  }
  ...
  Future<void> _refreshReplies() async {
    ...
    try {
      final all = await MakeChessMessageRealtimeService.instance.syncFromDatabase();
      ...
      final fixedCaseIds = replies
          .where((message) => '${message.metadata['replyStatus'] ?? ''}' == 'fixed')
  ...
  Future<void> _sendMessageToTarget({
    ...
    if (user == null) {
      throw StateError(_t('Для административного сообщения требуется вход в аккаунт'));
    }
    ...
    await MakeChessMessageRealtimeService.instance.send(
      MakeChessMessage(
        ...
        category: switch (action) {
          ...
          'restriction' || 'lift_restrictions' || 'keep_restrictions' =>
            'admin_restriction',
        ...
        title: switch (action) {
          ...
          'delete' || 'restore' || 'keep_deleted' => 'Административное удаление',
      ...
      final result = await showDialog<({String text, DateTime? dueAt})>(
        ...
        builder: (dialogContext) => StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            final days =
                (int.tryParse(daysController.text.trim()) ?? 10).clamp(1, 365).toInt();
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      if (action == 'restriction' ||
                        ...
                        const SizedBox(height: 5),
                        Text(_restrictionSummary(), style: AppTextStyles.bodyDim),
                      ],
                    ],
                  ),
                ),
  ...
  String _restrictionSummary() {
    ...
    final selected = _restrictionDraft.entries
        ...
        .toList(growable: false);
    return selected.isEmpty ? _t('Ограничения не выбраны') : selected.join(' • ');
  }
  ...
  Widget _reviewDecisionButtons(AdminCaseRecord record) {
    final replies = _caseReplies
        .where(
          (message) =>
              '${message.metadata['adminCaseId'] ?? ''}' == record.id,
        )
        .toList(growable: false);
        ...
        Expanded(
          child: Row(
            ...
            children: [
              ...
              Expanded(child: _selected == null ? _emptySelection() : _workspace()),
            ],
          ),
        ),
      ],
                    ...
                    itemBuilder: (_, index) {
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          padding: const EdgeInsets.all(10),
                          decoration:
                              AppDecorations.neoButton(active: active),
                          child: Row(
                            children: [
                              CircleAvatar(
                                radius: 17,
  ...
  Widget _actionButtons() {
    ...
    return Wrap(
      ...
      children: [
        ...
        OutlinedButton.icon(
          onPressed:
              disabled ? null : () => _openActionDialog('restriction'),
  ...
  Widget _historyZone() {
    ...
    final timeline = <({DateTime time, String actor, String type, String text})>[];
...
class _AdminArchivePanelState extends State<AdminArchivePanel> {
          ...
          Container(
            ...
            child: Wrap(
              ...
              children: [
                ...
                _archiveSwitch(
                    'schoolsTeachers', 'Сохранять школы и учителей'),
              ],
            ),
          ),
        ],
  ...
  Widget _archiveBody() {
    ...
    final rows = _cases
        .where((item) => allowedKinds.contains(item.targetKind))
        .toList();
              ...
              Row(
                children: [
                  ...
                  IconButton(
                    ...
                    onPressed: () => setState(
                      () => _archiveDeleteConfirmId =
                          confirming ? null : item.id,
                    ),
                    icon: const Icon(
