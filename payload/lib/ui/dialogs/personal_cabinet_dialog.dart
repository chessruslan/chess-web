...
class _PersonalCabinetDialog extends StatefulWidget {
  ...
  @override
  State<_PersonalCabinetDialog> createState() => _PersonalCabinetDialogState();
}
...
class _PersonalCabinetDialogState extends State<_PersonalCabinetDialog> {
  ...
  Future<void> _loadLichess() async {
    setState(() => _lichessLoading = true);
    try {
      final value = await LichessService.instance.status();
      if (mounted) setState(() => _lichess = value);
    } catch (_) {
      if (mounted) {
        setState(() => _lichess = const LichessConnection(connected: false));
      }
    } finally {
  ...
  Future<void> _connectLichess() async {
    ...
    try {
      ...
      if (!mounted) return;
      setState(() => _lichessLoading = false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: MakeChessLocalizedText(
                'Не удалось подключить Lichess: $error')),
      );
    }
  }
  ...
  Future<void> _disconnectLichess() async {
    setState(() => _lichessLoading = true);
    try {
      await LichessService.instance.disconnect();
      if (mounted) {
        setState(() => _lichess = const LichessConnection(connected: false));
      }
    } finally {
  ...
  Future<void> _load() async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
            content: MakeChessLocalizedText(
                'Не удалось открыть личный кабинет: $error')),
      );
    }
  }
  ...
  Future<void> _editProfile() async {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
          content: MakeChessLocalizedText('Личные данные сохранены локально')),
    );
  }
  ...
  Future<void> _deleteGame(CabinetGameRecord game) async {
    final confirmed = await showDialog<bool>(
          ...
          builder: (dialogContext) => AlertDialog(
            title: const MakeChessLocalizedText('Удалить партию?'),
            content: MakeChessLocalizedText(
      ...
      builder: (dialogContext) => AlertDialog(
        ...
        actions: [
          TextButton.icon(
            ...
            label: const MakeChessLocalizedText('Скопировать PGN'),
          ),
          FilledButton(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          const MakeChessLocalizedText('Личный кабинет не загрузился',
              style: AppTextStyles.sectionTitle),
          const SizedBox(height: 12),
          FilledButton(
              onPressed: _load,
              child: const MakeChessLocalizedText('Повторить')),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const Expanded(
            child: MakeChessLocalizedText('Личный кабинет',
                style: AppTextStyles.sectionTitle),
          ),
          MakeChessLocalizedText(
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                MakeChessLocalizedText(profile.fullName,
                    style: AppTextStyles.sectionTitle),
                const SizedBox(height: 3),
                MakeChessLocalizedText('@${profile.nickname}',
                    style: AppTextStyles.bodyDim),
                ...
                if (profile.about.trim().isNotEmpty) ...[
                  ...
                  MakeChessLocalizedText(profile.about,
          ...
          FilledButton.icon(
            ...
            label: const MakeChessLocalizedText('Редактировать'),
          ),
        ],
      ),
    );
  ...
  Widget _info(IconData icon, String text, {String? empty}) {
    ...
    return Row(
      ...
      children: [
        ...
        MakeChessLocalizedText(value, style: AppTextStyles.caption),
      ],
    );
  }
            ...
            return SizedBox(
              ...
              child: Container(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    MakeChessLocalizedText(type.title,
                    ...
                    const SizedBox(height: 3),
                    MakeChessLocalizedText('$count сохранённых партий',
                        style: AppTextStyles.caption),
                  ],
                ),
              ),
    ...
    return Wrap(
      ...
      children: _CabinetSection.values.map((section) {
        ...
        return FilledButton.tonalIcon(
          ...
          label: MakeChessLocalizedText(section.title),
        ...
        children: [
          Row(
            children: [
              ...
              MakeChessLocalizedText('${games.length}',
                  style: AppTextStyles.bodyDim),
            ],
          ),
          const SizedBox(height: 10),
          ...
          const SizedBox(height: 12),
          if (games.isEmpty) _emptyArchive(type) else ...games.map(_gameCard),
        ],
      ),
    );
  }
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          MakeChessLocalizedText('В разделе «${type.title}» пока нет партий',
              style: AppTextStyles.body),
        ...
        children: [
          ...
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                MakeChessLocalizedText('${game.whiteName} — ${game.blackName}',
          ...
          TextButton.icon(
            ...
            label: const MakeChessLocalizedText('PGN'),
          ),
          IconButton(
            tooltip: MakeChessLocalization.phrase('Удалить'),
  ...
  Widget _buildSettings(CabinetProfile profile) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          const MakeChessLocalizedText('Настройки личного кабинета',
              style: AppTextStyles.panelTitle),
          const SizedBox(height: 8),
          Container(
            padding: const EdgeInsets.all(12),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const MakeChessLocalizedText('Аккаунт Lichess',
                          style: AppTextStyles.body),
                      const SizedBox(height: 3),
                      MakeChessLocalizedText(
                        _lichess?.connected == true
                            ? 'Подключён: ${_lichess?.username}'
                            : 'Подключите аккаунт один раз для игры через MakeChess',
                        style: AppTextStyles.caption,
                      ),
                    ],
                  ),
                ...
                else if (_lichess?.connected == true)
                  OutlinedButton(
                    onPressed: _disconnectLichess,
                    child: const MakeChessLocalizedText('Отключить'),
                  )
                else
                  FilledButton(
                    onPressed: _connectLichess,
                    child: const MakeChessLocalizedText('Подключить Lichess'),
                  ),
              ],
            ),
          ),
...
class _EditCabinetProfileDialogState extends State<_EditCabinetProfileDialog> {
    ...
    try {
      ...
      if (file.size > 1500 * 1024) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content:
                MakeChessLocalizedText('Фотография должна быть меньше 1,5 МБ'),
          ),
        );
        return;
      }
  ...
  void _save() {
    ...
    if (nickname.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: MakeChessLocalizedText('Введите ник')),
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      backgroundColor: AppColors.surface,
      title: const MakeChessLocalizedText('Редактировать профиль',
          style: AppTextStyles.sectionTitle),
      content: SizedBox(
        width: 820,
        child: SingleChildScrollView(
            ...
            children: [
                    ...
                    child: Column(
                      ...
                      children: [
                        FilledButton.tonalIcon(
                          ...
                          label: MakeChessLocalizedText(_pickingPhoto
                              ? 'Загрузка…'
                        ...
                        TextButton.icon(
                          ...
                          label:
                              const MakeChessLocalizedText('Убрать фотографию'),
                        ),
                        const MakeChessLocalizedText(
              ...
              TextField(
                ...
                decoration: AppInputs.dark(
                    labelText: MakeChessLocalization.phrase('Кратко о себе')),
              ),
            ],
          ),
        ),
