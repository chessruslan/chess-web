...
class _CallOverlayState extends State<CallOverlay> {
  ...
  void _toast(String msg) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: MakeChessLocalizedText(msg)));
  }
  ...
  Widget _buildTopBar(BuildContext context) {
    return Row(
      children: [
        ...
        const SizedBox(width: 12),
        FilledButton(onPressed: _create, child: const MakeChessLocalizedText('Создать')),
        const SizedBox(width: 8),
        FilledButton.tonal(
            onPressed: _join, child: const MakeChessLocalizedText('Присоединиться')),
        const SizedBox(width: 8),
        IconButton(
                    ...
                    final tile = ListTile(
                      ...
                      onTap: () {
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                              content: MakeChessLocalizedText('Room ID: ${u.username} выбран')),
                        );
                      },
                      trailing: TextButton(
                        onPressed: () {
